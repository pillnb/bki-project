...
import {
  groupSubmissionsByParentId,
  getTotalPages,
} from "@/lib/utils/sertifikatGrouping";
...
export default function AdminDashboardPage() {
  ...
  const [filter, setFilter] = useState<"PENDING_APPROVAL" | "all">(
    "PENDING_APPROVAL"
  );
  ...
  const [cancellingGroupId, setCancellingGroupId] = useState<number | null>(
    null
  );
  const [showCancelModalFor, setShowCancelModalFor] = useState<number | null>(
    null
  );
  const [cancelReason, setCancelReason] = useState<string>("");
  const [counterYear, setCounterYear] = useState<string>(
    ...
    try {
      ...
      if (filter === "all") {
        const qs = selectedStatuses.length
          ? `?statuses=${encodeURIComponent(selectedStatuses.join(","))}`
          : "";
        url += qs;
      }
  ...
  const loadCounter = async (tahun: string) => {
    ...
    try {
      const res = await fetch(
        `/api/sertifikat/admin/counter?tahun=${encodeURIComponent(tahun)}`
      );
  ...
  const setCounter = async () => {
    if (
      !Number.isFinite(counterValue as number) ||
      (counterValue as number) < 0
    )
      return alert("Masukkan angka valid >= 0");
    setCounterLoading(true);
    try {
      const res = await fetch("/api/sertifikat/admin/counter", {
        ...
        body: JSON.stringify({
          tahun: counterYear,
          sequence: Number(counterValue),
          useAsNext: counterUseAsNext,
        }),
      });
      if (!res.ok) {
  ...
  const cancelGroup = async (rootId: number, reason?: string) => {
    const ok = true; // confirmation handled by modal
    if (!ok) return;
    setCancellingGroupId(rootId);
    try {
      const res = await fetch(`/api/sertifikat/admin/cancel/${rootId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ keterangan: reason || "" }),
      });
      if (!res.ok) {
        const txt = await res.text().catch(() => "Gagal membatalkan");
        throw new Error(txt || "Gagal membatalkan");
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    a.href = url;
                    a.download = `sertifikat_export_${new Date()
                      .toISOString()
                      .slice(0, 10)}.xlsx`;
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setSelectedStatuses((s) => {
                          if (e.target.checked)
                            return Array.from(new Set([...s, opt.value]));
              ...
              return (
                <div key={idx} className="bg-white rounded-xl shadow-lg p-6">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    {/* Cancel group button (show once per card if approved) */}
                    {first.status === "APPROVED" && (
                      <div className="ml-4">
                        <button
                          onClick={() =>
                            setShowCancelModalFor(first.parentId || first.id)
                          }
                          className="px-3 py-2 rounded-md bg-red-600 hover:bg-red-500 text-white text-sm font-semibold"
                          title="Cancel grup sertifikat"
                        >
                          Cancel
                        </button>
                      </div>
                  ...
                  {/* Show nomor sertifikat list for APPROVED and CANCEL */}
                  {(first.status === "APPROVED" ||
                    first.status === "CANCEL") && (
                    <div
                      className={`${
                      ...
                      <div className="space-y-2">
                        {group.map((item) =>
                          item.nomorSertifikat ? (
                            ...
                            >
                              <p
                                className={`text-xs font-mono break-all flex-1 mr-4 ${
                                  item.status === "CANCEL"
                                    ? "text-red-600"
                                    : "text-gray-800"
                                }`}
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  onClick={async () => {
                                    if (
                                      !confirm(
                                        "Hapus sertifikat ini? Nomor urut akan dikembalikan."
                                      )
                                    )
                                      return;
                                    try {
                                      const res = await fetch(
                                        `/api/sertifikat/admin/delete/${item.id}`,
                                        { method: "POST" }
                                      );
                                      if (!res.ok) {
                                        const j = await res
      ...
      {cancellingGroupId && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full text-center">
            <h3 className="text-lg font-semibold mb-2">Memproses...</h3>
            <p className="text-sm text-gray-600">
              Mohon tunggu, pembatalan sedang diproses.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full">
            ...
            <div className="mb-4">
              ...
              />
              {counterLoading && (
                <div className="text-sm text-gray-500 mt-2">Memuat...</div>
              )}
            </div>
            ...
            <div className="mb-4 flex items-center gap-3">
              <input
                id="useAsNext"
                type="checkbox"
                checked={counterUseAsNext}
                onChange={(e) => setCounterUseAsNext(e.target.checked)}
              />
              <label htmlFor="useAsNext" className="text-sm text-gray-700">
                Gunakan sebagai nomor berikutnya
              </label>
            </div>
      ...
      {/* Cancel Reason Modal */}
      {showCancelModalFor !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full">
            <h3 className="text-xl font-bold text-gray-800 mb-4">
              Batalkan Grup Sertifikat
            </h3>

            <p className="text-sm text-gray-600 mb-3">
              Masukkan keterangan pembatalan (opsional). Keterangan ini akan
              disimpan pada semua halaman dalam grup.
            </p>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() =>
                  cancelGroup(showCancelModalFor as number, cancelReason)
                }
                className="flex-1 bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-lg font-semibold transition"
              >
                Batalkan Grup
